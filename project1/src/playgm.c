// ox Othello GUI for openwindows, 26.7.91 -
...
void Auto_WARTEN(void)
{
  ...
  // make both players wait

  if (!Break1) { 
    FOR (i, 5) { 
      if (f_pipes) {
	if (PipeSyncSendBREAK(TO_1)) break; else WARTEN(3); 
      } else {
	if (SyncSendBREAK(TO_1)) break; else WARTEN(3); 
      }
    } 
    if (i >= 5) printf(">>> SP_COMP1 doesn't respond\n");
    Break1 = TRUE; 
  }

  if (!Break2 && strcmp(TO_1, TO_2)) { 
    FOR (i, 5) { 
      if (f_pipes) {
	if (PipeSyncSendBREAK(TO_2)) break; else WARTEN(3); 
      } else {
	if (SyncSendBREAK(TO_2)) break; else WARTEN(3); 
      }
    }
    if (i >= 5) printf(">>> SP_COMP2 doesn't respond\n");
    Break2 = TRUE; 
  }
  ...
  if (f_pipes) {
    //PipeReceive(FROM_1, Nachricht); 
    //PipeReceive(FROM_2, Nachricht); 
  } else {
...
int SendInfo(char *channel, int player, int time, SPIELINFO *pInfo, int MoveNr)
{
  ...
  if (!f_old && SfGleich(&sf, &Spiel[1].Sf)) {
    ...
    if (f_pipes) 
      return PipeSyncSendGAME(channel, player, time, &Game, 1);
    else
    ...
    if (f_pipes) {
      return PipeSyncSendBOARD(channel, player, time, 
...
#if SAVE_POS
    SaveGame("/tmp/ox.osp", Spiel, ZugNr);
#endif
